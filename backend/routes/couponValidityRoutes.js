...
router.post('/coupon-validities', async (req, res) => {
  try {
    ...
    const { couponIndex, datetime_stamp_start, datetime_stamp_end } = req.body;

    if (
      couponIndex === undefined ||
      !datetime_stamp_start ||
      !datetime_stamp_end ||
      isNaN(couponIndex)
    ) {
      return res.status(400).json({
        message: 'couponIndex (number), datetime_stamp_start, and datetime_stamp_end are required.',
      });
    }
    ...
    const parsedStartDate = parseDateStringAsLocal(datetime_stamp_start);
    const parsedEndDate = parseDateStringAsLocal(datetime_stamp_end);
    ...
    if (parsedStartDate >= parsedEndDate) {
      return res.status(400).json({
        message: 'datetime_stamp_start must be earlier than datetime_stamp_end.',
      });
    }
